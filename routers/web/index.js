module.exports = app => {
    ...
    // 新闻获取
    router.get('/newsinit', async (req, res) => {
        const parent = await Category.findOne().where({
            name: '新闻分类'
        })
        const categories = await Category.find().where({ parent }).lean()
        const newsTitles = ["7月10日体验服停机更新公告", "7月10日全服不停机更新公告", "7月9日版本更新异常问题说明", "英雄平衡性调整丨鬼谷子加强，玄策/干将等调优", "新皮肤爆料丨一念通天，神魔无惧，李信世冠新装即将登场！", "铂金铁盘披萨·【王者】光环降临！限时买一送一！", "保利剧院公司、保利演艺携手腾讯新文创、腾讯游戏王者荣耀，共同探索文化演艺新边界", "英雄调整情报丨鬼谷子加强，玄策/干将优化", "新皮肤爆料丨一念通天，神魔无惧，李信世冠新装即将登场！", "正式服预告⑥ | 新展示背景、新地图氛围，三分奇兵即将上线", "7月10日体验服停机更新公告", "夏日主题荣耀播报票选活动开启公告", "7月10日全服不停机更新公告", "7月9日版本更新异常问题说明", "【已开服】7月9日正式服“三分奇兵”版本更新公告", "【三分之战 . 集奇兵制胜】活动开启公告", "S20赛季开启，缤纷好礼等你来拿", "【赛季冲刺，新版本探秘】活动公告", "抢先服更新  S20赛季来袭", "【赛季冲刺，新版本探秘】活动FAQ", "2020年王者荣耀世界冠军杯小组赛赛程出炉", "2020年王者荣耀世界冠军杯第二轮选拔赛 加赛及抽签规则", "世冠小组赛抽签仪式6月28日现场直播，小组赛之战一触即发！", "2020年王者荣耀世界冠军杯（KCC）赛事规则", "无惧挑战向阳而生，TS冠军之夜今日18:00惊喜来袭"]
    ...
    router.get('/newslist', async (req, res) => {
        // const cats=await Category.find({name:'新闻分类'}).populate({
        //     path:'children',
        //     populate:{
        //         path:'newsList'
        //     }
        // }).lean()
        // const newsList=cats.pop().children
    ...
    router.get('/heroList', async (req, res) => {
        ...
        const subCats = heroes.map(v => v._id)
        heroes.unshift({
            name: '热门',
